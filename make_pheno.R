...
t1d.pheno <- data.frame(eid = icd$eid)
t2d.pheno <- data.frame(eid = icd$eid)
...
write.table(t1d.pheno, file = "t1d.pheno", sep = "\t", row.names = FALSE)
write.table(t2d.pheno, file = "t2d.pheno", sep = "\t", row.names = FALSE)