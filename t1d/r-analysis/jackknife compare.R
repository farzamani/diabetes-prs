...
for (model in models) {
    score <- read.csv(file.path(getwd(), "results", "compare", model, "jackknife.jack"), header = T,  sep = " ")
    score$model <- model
...
plot1 <- ggplot(df_corr, aes(x = model, y = Estimate, fill = model)) +
  ...
  labs(title = "Correlation",
      subtitle = "PRS of T1D to Predict T2D",
       x = "Model",
       y = "Estimate") +
...
# Open a PNG graphics device for saving the plot
png("figures/t1d_jackknife_combined_compare.png", width = 10, height = 4, units = "in", res = 300)
