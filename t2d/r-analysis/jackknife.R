...
for (model in models) {
    score <- read.csv(file.path(getwd(), "results gcta", "megaprs", model, "jackknife.jack"), header = T,  sep = " ")
    score$model <- model
...
plot1 <- ggplot(df_corr, aes(x = model, y = Estimate, fill = model)) +
  ...
  geom_hline(yintercept = 0.085358, linetype="dotted", size = 0.8) +
...
plot2 <- ggplot(df_liability, aes(x = model, y = Estimate, fill = model)) +
  ...
  geom_errorbar(aes(ymin = Estimate - SD, ymax = Estimate + SD), 
                width = .2, position = position_dodge(.9)) +
  geom_hline(yintercept = 0.029199, linetype="dotted", size = 0.8) +
  geom_text(aes(label = sprintf("%.5f", Estimate)), vjust = 5, position = position_dodge(0.9), size = 2) +
...
plot3 <- ggplot(df_auc, aes(x = model, y = Estimate, fill = model)) +
  ...
  geom_errorbar(aes(ymin = Estimate - SD, ymax = Estimate + SD), 
                width = .2, position = position_dodge(.9)) +
  geom_hline(yintercept = 0.592755, linetype="dotted", size = 0.8) +
  geom_text(aes(label = sprintf("%.5f", Estimate)), vjust = 5, position = position_dodge(0.9), size = 2) +
...
# Open a PNG graphics device for saving the plot
png("figures/t2d_jackknife_combined_gcta.png", width = 10, height = 4, units = "in", res = 300)
